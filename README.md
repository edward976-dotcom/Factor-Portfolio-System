...
這是一個以 R Shiny 開發的**華量化投資組合分析與多因子選股儀表板**。本系統提供互動式的 N 檔資產配置策略、蒙地卡羅萬組投組模擬、風險平價貢獻度分析，以及對比大盤的動態歷史回測。
...
- **基準大盤對沖 (Benchmark Comparison)**：將您的投組累積財富對比大盤基準 (如 `^TWII` 或 `SPY`)，評估是否成功創造超額報酬 (Alpha)。
...
### 執行方法
1. 下載本專案至本機。
2. 啟動 `0315.R` (或 `app.R` 根據您的命名)。
3. 在 RStudio 中點選 `Run App`，即可開啟完整的互動式儀表板。

## 📉 注意事項 (Disclaimer)
本專案為開源專案，提供量化技術與介面開發展示用。系統計算之回測績效與最佳化權重均基於歷史數據，**不代表未來投資收益之保證**。本工具非構成任何財務投資建議。
